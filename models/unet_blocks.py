...
from .mid_resnet3d import ResnetBlock3D, Upsample3D, Downsample3D
from .mid_attention3d import Transformer3DModel, TransformerTemporalModel
...
def transformer_g_c(transformer, sample):
    sample = g_c(custom_checkpoint(transformer, mode='temp'),
                 sample, use_reentrant=False,
                 )
    return sample
...
def up_down_g_c(resnet, temp_conv, hidden_states, temb):
...
class Temporal3DConvLayer(nn.Module):
    """
...
class UNetMidBlock3DCrossAttn(nn.Module):
    def __init__(
        ...
        temp_convs = [Temporal3DConvLayer(in_channels, in_channels, dropout=0.1)]
        ...
        for _ in range(num_layers):
            ...
            temp_convs.append(
                Temporal3DConvLayer(in_channels, in_channels, dropout=0.1)
            )
            temp_convs[-1].convs[-1] = zero_module(temp_convs[-1].convs[-1])
...
class DownBlock3D(nn.Module):
    ...
    def forward(self, hidden_states, temb=None):
        ...
        for resnet, temp_conv in zip(self.resnets, self.temp_convs):
            if self.gradient_checkpointing:
                hidden_states = up_down_g_c(resnet, temp_conv, hidden_states, temb)
            else:
                hidden_states = resnet(hidden_states, temb)
                hidden_states = temp_conv(hidden_states)
